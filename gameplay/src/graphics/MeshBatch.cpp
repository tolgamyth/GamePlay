...
namespace gameplay
{
  ...
  MeshBatch* MeshBatch::create(const VertexFormat& vertexFormat, Mesh::PrimitiveType primitiveType, const char* materialPath, bool indexed, unsigned int initialCapacity, unsigned int growSize)
  {
    ...
    MeshBatch* batch = create(vertexFormat, primitiveType, material.get(), indexed, initialCapacity, growSize);
    return batch;
  }
  ...
  void MeshBatch::updateVertexAttributeBinding()
  {
    ...
    {
      std::shared_ptr<Technique> t = _material->getTechniqueByIndex(i);
      assert(t);
      for (unsigned int j = 0, passCount = t->getPassCount(); j < passCount; ++j)
      {
        std::shared_ptr<Pass> p = t->getPassByIndex(j);
        assert(p);
        std::shared_ptr<VertexAttributeBinding> b = VertexAttributeBinding::create(_vertexFormat, _vertices, p->getEffect());
