...
namespace gameplay
{
  ...
  Scene* SceneLoader::loadInternal(const char* url)
  {
    ...
    if (activeCamera)
    {
      std::shared_ptr<Node> camera = _scene->findNode(activeCamera);
  ...
  void SceneLoader::applyNodeProperty(SceneNode& sceneNode, std::shared_ptr<Node> node, const Properties* sceneProperties, const SceneNodeProperty& snp)
  {
      ...
      snp._type == SceneNodeProperty::TEXT)
    {
      ...
      case SceneNodeProperty::MATERIAL:
      {
        ...
        if (model)
        {
          std::shared_ptr<Material> material = Material::create(p);
      ...
      case SceneNodeProperty::COLLISION_OBJECT:
      {
        ...
        else
        {
          ...
          if (name)
          {
            std::shared_ptr<Node> modelNode = _scene->findNode(name);
            if (!modelNode)
            {
            ...
            {
              if (dynamic_cast<Model*>(modelNode->getDrawable().get()) == nullptr)
              {
  ...
  void SceneLoader::applyNodeUrls(SceneNode& sceneNode, std::shared_ptr<Node> parent)
  {
    ...
    for (int j = (int)sceneNode._properties.size() - 1; j >= 0; --j)
    {
      ...
      if (file.empty())
      {
        ...
        {
          std::shared_ptr<Node> node = parent ? parent->findNode(id.c_str()) : _scene->findNode(id.c_str());
          if (node)
          {
        ...
        else
        {
          ...
          if (nodeCount > 0)
          {
            for (unsigned int k = 0; k < nodeCount; ++k)
            {
              ...
              std::shared_ptr<Node> node = nodes[k];
      ...
      else
      {
        ...
        if (tmpBundle)
        {
          if (sceneNode._exactMatch)
          {
            std::shared_ptr<Node> node = tmpBundle->loadNode(id.c_str(), _scene);
            if (node)
            {
              node->setId(sceneNode._nodeID);
          ...
          else
          {
            ...
            for (unsigned int k = 0; k < objectCount; ++k)
            {
              ...
              if (strstr(objid, id.c_str()) == objid)
              {
                ...
                std::shared_ptr<Node> node = tmpBundle->loadNode(objid);
                if (node)
                {
                  // Construct a new node ID using _nodeID plus the remainder of the partial match.
                  std::string newID(sceneNode._nodeID);
    ...
    if (!hasURL)
    {
      ...
      std::shared_ptr<Node> node = parent ? parent->findNode(sceneNode._nodeID) : _scene->findNode(sceneNode._nodeID);
      if (node)
      {
        sceneNode._nodes.push_back(node);
      }
    ...
    {
      std::shared_ptr<Node> parent = sceneNode._nodes[i];
  ...
  void SceneLoader::createAnimations()
  {
    ...
    for (size_t i = 0, count = _animations.size(); i < count; i++)
    {
      ...
      std::shared_ptr<Node> node = _scene->findNode(_animations[i]._targetID);
      if (!node)
      {
  ...
  void SceneLoader::loadPhysics(Properties* physics)
  {
    ...
    while ((constraint = physics->getNextNamespace()) != nullptr)
    {
      if (strcmp(constraint->getNamespace(), "constraint") == 0)
      {
        ...
        std::shared_ptr<Node> rbANode = _scene->findNode(name);
        if (!rbANode)
        {
        ...
        if (name)
        {
          std::shared_ptr<Node> rbBNode = _scene->findNode(name);
          if (!rbBNode)
          {
