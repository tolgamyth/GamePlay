...
namespace gameplay
{
  ...
  Scene* Bundle::loadScene(const char* id)
  {
    ...
    if (childrenCount > 0)
    {
      ...
      {
        std::shared_ptr<Node> node = readNode(scene, nullptr);
        if (node)
        {
          scene->addNode(node);
        }
    ...
    {
      std::shared_ptr<Node> node = scene->findNode(xref.c_str() + 1, true);
      assert(node);
  ...
  std::shared_ptr<Node> Bundle::loadNode(const char* id)
  {
    return loadNode(id, nullptr);
  }

  std::shared_ptr<Node> Bundle::loadNode(const char* id, Scene* sceneContext)
  {
    assert(id);
    assert(_references);
    assert(_stream);
    ...
    _trackedNodes = new std::map<std::string, std::shared_ptr<Node>>();
    std::shared_ptr<Node> node = loadNode(id, sceneContext, nullptr);
    if (node)
      ...
      if (ref->type == BUNDLE_TYPE_ANIMATIONS)
      {
        ...
        for (unsigned int j = 0; j < animationCount; j++)
        {
          ...
          for (unsigned int k = 0; k < animationChannelCount; k++)
          {
            ...
            std::map<std::string, std::shared_ptr<Node>>::iterator iter = _trackedNodes->find(targetId);
  ...
  std::shared_ptr<Node> Bundle::loadNode(const char* id, Scene* sceneContext, std::shared_ptr<Node> nodeContext)
  {
    assert(id);
  ...
  std::shared_ptr<Node> Bundle::readNode(Scene* sceneContext, std::shared_ptr<Node> nodeContext)
  {
    const char* id = getIdFromOffset();
    assert(id);
    assert(_stream);
    ...
    if (_trackedNodes)
    {
      std::map<std::string, std::shared_ptr<Node>>::iterator iter = _trackedNodes->find(id);
      if (iter != _trackedNodes->end())
      {
        ...
        if (materialCount > 0)
        {
          for (unsigned int i = 0; i < materialCount; ++i)
          {
            ...
            if (materialPath.length() > 0)
            {
              ...
              std::shared_ptr<Material> material = Material::create(materialPath.c_str());
              if (material)
              {
  ...
  void Bundle::resolveJointReferences(Scene* sceneContext, std::shared_ptr<Node> nodeContext)
  {
    assert(_stream);
    ...
    for (size_t i = 0, skinCount = _meshSkins.size(); i < skinCount; ++i)
    {
      ...
      for (size_t j = 0; j < jointCount; ++j)
      {
        ...
        if (jointId.length() > 1 && jointId[0] == '#')
        {
          ...
          std::shared_ptr<Node> n = loadNode(jointId.c_str(), sceneContext, nodeContext);
          if (n && n->getType() == Node::JOINT)
          {
            std::shared_ptr<Joint> joint = std::dynamic_pointer_cast<Joint>(n);
      ...
      if (jointCount > 0)
      {
        std::shared_ptr<Joint> rootJoint = skinData->skin->getJoint((unsigned int)0);
        std::shared_ptr<Node> node = rootJoint;
        assert(node);
        ...
        while (true)
        {
          if (parent)
          {
            if (skinData->skin->getJointIndex(std::dynamic_pointer_cast<Joint>(parent)) != -1)
            {
              // Parent is a joint in the MeshSkin, so treat it as the new root.
              rootJoint = std::dynamic_pointer_cast<Joint>(parent);
            }
  ...
  Animation* Bundle::readAnimationChannel(Scene* scene, Animation* animation, const char* animationId)
  {
    ...
    if (!target)
    {
      target = scene->findNode(targetId.c_str()).get();
      if (!target)
      {
  ...
  void Bundle::setTransform(const float* values, std::shared_ptr<Transform> transform)
  {
    assert(transform);
