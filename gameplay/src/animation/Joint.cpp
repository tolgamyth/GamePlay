...
namespace gameplay
{
  ...
  std::shared_ptr<Joint> Joint::create(const char* id)
  {
    return std::make_shared<Joint>(id);
  }

  std::shared_ptr<Node> Joint::cloneSingleNode(NodeCloneContext& context) const
  {
    std::shared_ptr<Joint> copy = Joint::create(getId());
    assert(copy);
  ...
  Scene* Joint::getScene() const
  {
    ...
    for (const SkinReference* itr = &_skin; itr && itr->skin; itr = itr->next)
    {
      ...
      if (model)
      {
        std::shared_ptr<Node> node = model->getNode();
        if (node)
        {
