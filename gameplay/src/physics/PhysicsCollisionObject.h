...
#define PHYSICS_COLLISION_MASK_DEFAULT btBroadphaseProxy::AllFilter
  ...
  protected:
     ...
     * Constructor.
     */
    PhysicsCollisionObject(std::shared_ptr<Node> node, int group = PHYSICS_COLLISION_GROUP_DEFAULT, int mask = PHYSICS_COLLISION_MASK_DEFAULT);
  ...
  private:
    ...
    public:
       ...
       */
      PhysicsMotionState(std::shared_ptr<Node> node, PhysicsCollisionObject* collisionObject, const Vector3* centerOfMassOffset = nullptr);
